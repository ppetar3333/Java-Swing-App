...
public class ProzorZaPrikazKombinovanePretrage extends JFrame {

    private DefaultTableModel table_model;
    private JTable vozaciTabela;


    public ProzorZaPrikazKombinovanePretrage(DoublyLinkedList<Vozac> rezultatKombinovanePretrage) {
        setTitle("Prikaz pretrage");
        setSize(1050, 200);
        setDefaultCloseOperation(DISPOSE_ON_CLOSE);
        setLocationRelativeTo(null);
        initGUI(rezultatKombinovanePretrage);
    }

    private void initGUI(DoublyLinkedList<Vozac> rezultatKombinovanePretrage) {

        String[] zaglavnje = new String[] {"Korisnicko ime", "Ime", "Prezime", "Adresa", "Pol", "Broj telefona", "Plata", "Broj clanske karte", "Automobil"};
        Object[][] sadrzaj = new Object[rezultatKombinovanePretrage.size()][zaglavnje.length];
        for (int i = 0; i < rezultatKombinovanePretrage.size(); i++) {
            Vozac vozac = rezultatKombinovanePretrage.get(i);
            if(vozac.isObrisan()) {
